...
/* THIRTEEN arcs, deliberately. It started at 8, went to 22 when the USA and
   Africa legs were added, and 22 read as busy and distracting — Siba, 2026-09-02:
   "we need to find balance between what it was and what we have now". Thirteen
   is that balance: every city he named still has a line, and London is down from
   eleven arcs to six, so the hub stops reading as a fan.
   Cut and why: London→Abuja (Lagos already carries Nigeria, and the two dots sit
   almost on top of each other at this size), London→Johannesburg and
   Johannesburg→Nairobi (Cape Town now carries South Africa; Johannesburg keeps
   its dot), London→Houston and London→LA (both reach the USA via New York,
   which is the honest route anyway), Lagos→Cape Town, Manchester→Cape Town,
   Kingston→Accra, London→Kingston.
   Every city removed from the ARC list is still a POINT on the globe. Dots are
   quiet; it is the lines that were shouting. */
const ARCS = [
  // UK → Africa
  ...
  // Across the USA, and the leg that closes the triangle to Africa
  { startLat: 40.7, startLng: -74.0, endLat: 34.1, endLng: -118.2 },// New York → Los Angeles
  { startLat: 40.7, startLng: -74.0, endLat: 29.8, endLng: -95.4 }, // New York → Houston
  { startLat: 29.8, startLng: -95.4, endLat: 5.6, endLng: -0.2 },   // Houston → Accra
  // Caribbean, and the one line that carries the lower-left of the disc
...
/* ── Ring pulses — FOUR anchors only ──
   Was eight, which meant something was always pulsing somewhere and added to the
   busyness without adding meaning. One anchor per corridor: the UK hub, West
   Africa, the USA, southern Africa. */
const RINGS = [
